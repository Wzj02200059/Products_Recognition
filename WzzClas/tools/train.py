...
def main():
    ...
    # initial optimizer
    optimizer, scheduler = build_optimizer(options, net, metric_fc)

    assert options['loss'] in ['rzloss', 'celoss', 'focal_loss']
    if options['loss'] == 'rzloss':
        loss = rzloss(options['margin'], options['gamma'])
    elif options['loss'] == 'celoss':
        loss = nn.CrossEntropyLoss()
    elif options['loss'] == 'focal_loss':
        loss = FocalLoss(options['focal_gamma'])

    # training model
    trainer = Pairs_Trainer(options, trainloader, unknow_trainloader)
    validator = Pairs_Validator(options, testloader, unknow_valloader)

    criterion = nn.CrossEntropyLoss()
    criterion_tml = nn.TripletMarginLoss(margin=2.0, p=2)

    best_score = 0
    for epoch in tqdm(range(options['epoch'])):
        model, metric_fc = trainer.train_epoch(net, criterion, criterion_tml, metric_fc, optimizer, loss, epoch, options)
        # validator(model, metric_fc, loss, epoch, options)
        # # validation
